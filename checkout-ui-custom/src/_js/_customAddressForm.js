...
class fnsCustomAddressForm {
  ...
  loadScript() {
    $('body').append(
      `<script src="https://maps.googleapis.com/maps/api/js?key=${this.googleMapsApiKey}&language=en-US&libraries=places&callback=callbackMap"></script>`
    )
  }
  ...
  googleForm() {
    ...
    _this.gPlacesAutocomplete.addListener('place_changed', function() {
      ...
      const country = _countries.find(
        c =>
          c[0] ==
          place.address_components.filter(item => item.types[0] == 'country')[0]
            .short_name
      )[1]
  ...
  getCountries() {
    ...
    return _this.deliveryCountries.map(countryCode => {
      const _i18n = vtex.i18n[_this.lang]
        ? vtex.i18n[_this.lang]
        ...
        : 'This field is required.'
            ...
            <div class="v-custom-ship-info">
              <p class="input ship-number text ${
                _this.addressrules.number ? '' : 'hide'
      ...
      _this.locale.requiredField
        ...
        : 'This field is required.'
              ...
              <p class="input ship-postalCode required text"><label for="ship-postalCode">${
                vtex.i18n[_this.lang]
                  ? vtex.i18n[_this.lang].cart.postalCode
                  : 'Zip Code'
  ...
  validateAllFields() {
    ...
    _this.validate = true
    $('.vcustom--vtex-omnishipping-1-x-address input:required').each(function(
      i
    ) {
      if (this.value == '') {
        $(this).addClass('error')
  ...
  bind() {
    ...
    $('body').on('click', '.vtex-omnishipping-1-x-backToAddressList', function(
      e
    ) {
      $('body').removeClass(_this.BodyFormClasses.join(' '))
      _this.address.addressId = ''
    ...
    $('body').on('click', '.vtex-omnishipping-1-x-addressItemOption', function(
      e
    ) {
      _this.address.addressId = _this.orderForm.shippingData.availableAddresses[
        $(this).index()
      ]
