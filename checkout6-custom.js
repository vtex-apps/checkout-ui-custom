...
var applyMoreProducts = function () {
    ...
    $('<a href="#/orderform" class="_buy-btn">Checkout</div>').insertAfter('.cart-totalizers .accordion-group');
    return true;
}
$(applyMoreProducts);
...
(function ($, window, document, undefined) {
    ...
    var pricingTableTpl = function (p_row) {
        ...
        return '' +
            ...
            '</div>' +
            '<div class="_cart-body">' +
            '<div class="_cart-col00 _cart-shipping">' +
            '</div>' +
    ...
    var fixPricingTable = function () {
        $('.full-cart tr.product-item').each(function (ndx, item) {
            ...
            $(item).find('._cart-shipping').html($(item).find('.shipping-estimate-date:first'));
            ...
            $('tr.CustomTax.CustomTax--total.hide').removeClass().addClass('taxTotal');
    ...
    var pickupNotFind = function () {
        ...
        if (contentPickupNotFind.length > 0) {
            contentPickupNotFind.html("<div class='pickup-notfind'>We couldn't find any pickup points near this address.<br /> </div>");
            setTimeout(function () {
                $('#shipping-option-delivery').click();
                console.log('No pick up point');
            }, 4500);
        };
    };
...
//Subscription begins
(function () {
    function isSubscriptionAssembly(assembly) {
        return assembly.id && assembly.id.indexOf('vtex.subscription') !== -1
    }

    function getSubscriptionAssemblies(item) {
        return item.assemblies.filter(isSubscriptionAssembly)
    }

    function hasSubscriptonAssemblies(item) {
        return getSubscriptionAssemblies(item).length > 0
    }

    function isSubscriptionAttachment(attachment) {
        return attachment.name && attachment.name.indexOf('vtex.subscription') !== -1
    }

    function hasSubscriptionAttachments(item) {
        return item.attachments.some(isSubscriptionAttachment)
    }

    function isItSubscribed(orderForm, index) {
        if (orderForm.subscriptionData && orderForm.subscriptionData.subscriptions && orderForm.subscriptionData.subscriptions.length) {
            return !!orderForm.subscriptionData.subscriptions.find(function (item, i) {
    ...
    $(window).on('orderFormUpdated.vtex', function (ev, orderForm) {
        ...
        if (!items.some(hasSubscriptonAssemblies)) {
            return
        }

        items.forEach(function (item, index) {
            if (hasSubscriptionAttachments(item)) {
                return
            }

            var subscriptionAssemblies = getSubscriptionAssemblies(item);

            if (subscriptionAssemblies.length === 0) {
                return
            }

            var subscriptionAssembly = subscriptionAssemblies[0];

            vtexjs.checkout.addItemAttachment(index, subscriptionAssembly.id, subscriptionAssembly.inputValues, null, false);

            if (!isItSubscribed(orderForm, index)) {
                var frequency = subscriptionAssembly['inputValues']['vtex.subscription.key.frequency'];
                var interval = frequency.replace(/\D/gi, '');
                var periodicity = frequency.replace(/\d\s/gi, '').toUpperCase();
                var startDate = new Date().toISOString().split('T')[0];
                var endDate = new Date(new Date().setFullYear(new Date().getFullYear() + 3)).toISOString().split('T')[0]

                vtexjs.checkout.sendAttachment('subscriptionData', {
                    "subscriptions": [
                        {
                            "itemIndex": index,
                            "plan": {
...
(function (w, d, $) {
    var SOFTWARE = 'SOFTWARE'; // category name for software products
    var SHIPPING_DATA_SELECTOR = '#shipping-data'; // shipping data section
    var PAYMENT_ICON_SELECTOR = '.icon-credit-card'; // payment icon
    var NEW_CLASS_PAYMENT_ICON_SOFTWARE = 'icon-home'; // toggled class for payment icon when hiding shipping data

    var SHIPPING_HASH = '#/shipping',
        PAYMENT_HASH = '#/payment';

    function hasSelectedAddress(orderForm) {
        return orderForm.shippingData.selectedAddresses.length > 0 && orderForm.shippingData.selectedAddresses[0].street !== null;
    }

    function isSoftware(item) {
        var productCategories = item.productCategories;

        if (typeof productCategories == 'undefined') {
            return false;
        }

        return Object.keys(productCategories).some(function (key) {
            return productCategories[key].toUpperCase() === SOFTWARE;
        });
    }

    function hasOnlySoftware(orderForm) {
        var items = orderForm.items;
    ...
    function handleHashChange() {
        $(PAYMENT_ICON_SELECTOR).attr('class', NEW_CLASS_PAYMENT_ICON_SOFTWARE);

        if (window.location.hash === SHIPPING_HASH) {
            window.location.hash = PAYMENT_HASH;
        }
    }

    // remove the shipping data if has only software products in minicart
    $(w).on('orderFormUpdated.vtex', function (evt, orderForm) {
        if (hasOnlySoftware(orderForm) && hasSelectedAddress(orderForm)) {
            $(SHIPPING_DATA_SELECTOR).hide();

            $(w).on('hashchange', handleHashChange);
        }
    });

    $(w).on('load', function () {
        var observer = new MutationObserver(function () {
            if (hasOnlySoftware(w.vtexjs.checkout.orderForm)) {
                $('.srp-summary-result').hide();
            }
        });

        $('.totalizers-list').each(function () {
