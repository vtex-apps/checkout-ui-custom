...
When developing a new version of Checkout UI Custom, take care to compile the checkout files prior to opening a PR and/or publishing a new version of the app. This can be done by running `yarn build` in your CLI, either inside the app's root folder or in the `/checkout-ui-custom/` folder.
